...
import type { PreviewStory } from "@/app/api/brief-preview/route";

const TIER_BADGE = {
  critical: { label: "CRITICAL", cls: "bg-red-500/10 text-red-400 border border-red-500/20" },
  monitor:  { label: "MONITOR",  cls: "bg-blue-500/10 text-blue-400 border border-blue-500/20" },
};
...
export function LandingPage() {
  ...
  useEffect(() => {
    fetch("/api/brief-preview")
      .then((r) => r.json())
      .then((res: { data: PreviewStory[]; error: string | null }) => {
      ...
      <section className="max-w-[680px] mx-auto px-6 pt-20 pb-16">
        ...
        {status === "done" ? (
          <div className="bg-[#22C55E]/10 border border-[#22C55E]/20 rounded-lg px-5 py-4 text-[15px] text-[#22C55E] font-semibold max-w-[480px] mb-4">
            You&apos;re in — opening today&apos;s brief…
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex gap-2 max-w-[480px] mb-3">
            <input
              id="email-input"
              type="email"
        ...
        <section className="max-w-[680px] mx-auto px-6 pb-24">
          ...
          <div className="border border-t-0 border-white/[0.06]">
            {stories.map((story, idx) => {
              const badge = TIER_BADGE[story.tier];
              return (
                <div
                  key={story.id}
                  className={`px-6 py-7 ${idx < stories.length - 1 ? "border-b border-white/[0.06]" : ""}`}
                >
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[rgba(232,226,217,0.4)]">
                      {story.source}
                    </span>
                  ...
                  <h3 className="text-[18px] font-semibold text-[#F5F0E8] leading-snug tracking-[-0.015em] mb-3">
                    {story.headline}
                  </h3>

                  <p className="text-[14px] text-[rgba(232,226,217,0.6)] leading-relaxed">
                    {story.summary}
                  </p>
                </div>
              );
